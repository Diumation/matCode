...
%             %---------- < Q_KF must be set up appropriately > ------------%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             R_KF=0.1^2;                                                	%   Specify noise on alpha: enable if wanted  
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             
...
%             %---------- < Q_KF must be set up appropriately > ------------%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             P_cov_ini=diag([4000^(2) 4000^(2)]);                            %   Initial state covariance guess - Change if needed
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             
...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    if (k>3*butter_order+1)                                                                         %   If simulation has initialised
       ...
       %        %Define other power ratios
       if (k>50)
       P_r_filt_ratio(k,2)=((P_r_filt(k,1)))/(P_r_filt(50,1));
       end
       if (k>100)
       P_r_filt_ratio(k,3)=((P_r_filt(k,1)))/(P_r_filt(100,1));
       end
       if (k>200)
       P_r_filt_ratio(k,4)=((P_r_filt(k,1)))/(P_r_filt(200,1));
       end
       if (k>300)
       P_r_filt_ratio(k,5)=((P_r_filt(k,1)))/(P_r_filt(300,1));
       end
       if (k>400)
       P_r_filt_ratio(k,6)=((P_r_filt(k,1)))/(P_r_filt(400,1));
       end
       if (k>500)
       P_r_filt_ratio(k,7)=((P_r_filt(k,1)))/(P_r_filt(500,1));
       end
       if (k>600)
       P_r_filt_ratio(k,8)=((P_r_filt(k,1)))/(P_r_filt(600,1));
       end
       if (k>700)
       P_r_filt_ratio(k,9)=((P_r_filt(k,1)))/(P_r_filt(700,1));
       end
       if (k>800)
       P_r_filt_ratio(k,10)=((P_r_filt(k,1)))/(P_r_filt(800,1));
       end
    ...
    %   Filtering
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))              	%   If intersections have begun & first time filter is run
            ...
            ind=k;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %-------- < check the performance of the EKF (UKF) > ---------%
            %------------- < Design a new filer algorithm > --------------%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
            ...
            [x_state(:,10*k),P_cov(:,:,10*k),K_EKF_gain(:,10*k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
if (k>50)
            [x_state(:,10*k+1),P_cov(:,:,10*k+1),K_EKF_gain(:,10*k+1)]=KF_form(x_vec_all(50,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,2),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+1)=x_state(:,10*k);
            P_cov(:,:,10*k+1)=P_cov(:,:,10*k);
            K_EKF_gain(:,10*k+1)=K_EKF_gain(:,10*k);          
end
if (k>100)
            [x_state(:,10*k+2),P_cov(:,:,10*k+2),K_EKF_gain(:,10*k+2)]=KF_form(x_vec_all(100,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,3),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+2)=x_state(:,10*k+1);
            P_cov(:,:,10*k+2)=P_cov(:,:,10*k+1);
            K_EKF_gain(:,10*k+2)=K_EKF_gain(:,10*k+1);       
end
if (k>200)
            [x_state(:,10*k+3),P_cov(:,:,10*k+3),K_EKF_gain(:,10*k+3)]=KF_form(x_vec_all(200,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,4),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+3)=x_state(:,10*k+2);
            P_cov(:,:,10*k+3)=P_cov(:,:,10*k+2);
            K_EKF_gain(:,10*k+3)=K_EKF_gain(:,10*k+2);   
end 
if (k>300)
            [x_state(:,10*k+4),P_cov(:,:,10*k+4),K_EKF_gain(:,10*k+4)]=KF_form(x_vec_all(300,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,5),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+4)=x_state(:,10*k+3);
            P_cov(:,:,10*k+4)=P_cov(:,:,10*k+3);
            K_EKF_gain(:,10*k+4)=K_EKF_gain(:,10*k+3);   
end
if (k>400)
            [x_state(:,10*k+5),P_cov(:,:,10*k+5),K_EKF_gain(:,10*k+5)]=KF_form(x_vec_all(400,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,6),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+5)=x_state(:,10*k+4);
            P_cov(:,:,10*k+5)=P_cov(:,:,10*k+4);
            K_EKF_gain(:,10*k+5)=K_EKF_gain(:,10*k+4);
end
if (k>500)
            [x_state(:,10*k+6),P_cov(:,:,10*k+6),K_EKF_gain(:,10*k+6)]=KF_form(x_vec_all(500,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,7),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+6)=x_state(:,10*k+5);
            P_cov(:,:,10*k+6)=P_cov(:,:,10*k+5);
            K_EKF_gain(:,10*k+6)=K_EKF_gain(:,10*k+5);
end
if (k>600)
            [x_state(:,10*k+7),P_cov(:,:,10*k+7),K_EKF_gain(:,10*k+7)]=KF_form(x_vec_all(600,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,8),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+7)=x_state(:,10*k+6);
            P_cov(:,:,10*k+7)=P_cov(:,:,10*k+6);
            K_EKF_gain(:,10*k+7)=K_EKF_gain(:,10*k+6);
end
if (k>700)
            [x_state(:,10*k+8),P_cov(:,:,10*k+8),K_EKF_gain(:,10*k+8)]=KF_form(x_vec_all(700,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,9),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+8)=x_state(:,10*k+7);
            P_cov(:,:,10*k+8)=P_cov(:,:,10*k+7);
            K_EKF_gain(:,10*k+8)=K_EKF_gain(:,10*k+7);
end
if (k>800)
            [x_state(:,10*k+9),P_cov(:,:,10*k+9),K_EKF_gain(:,10*k+9)]=KF_form(x_vec_all(800,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,10),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+9)=x_state(:,10*k+8);
            P_cov(:,:,10*k+9)=P_cov(:,:,10*k+8);
            K_EKF_gain(:,10*k+9)=K_EKF_gain(:,10*k+8);
end
            time_comput_EKFPR(k)=toc ;
        ...
        elseif ((obs_check==1)&&(k>k_obs))                                  %   If intersections have begun & filter has alreay started
            ...
            %Extended Kalman Filter
            tic;
%            [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,k-1),P_cov(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);

            [x_state(:,10*k),P_cov(:,:,10*k),K_EKF_gain(:,10*k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,10*k-1),P_cov(:,:,10*k-1),F_KF,G_KF,Q_KF,R_KF);
if (k>50)
            [x_state(:,10*k+1),P_cov(:,:,10*k+1),K_EKF_gain(:,10*k+1)]=KF_form(x_vec_all(50,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,2),x_state(:,10*k),P_cov(:,:,10*k),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+1)=x_state(:,10*k);
            P_cov(:,:,10*k+1)=P_cov(:,:,10*k);
            K_EKF_gain(:,10*k+1)=K_EKF_gain(:,10*k);          
...
if (k>100)
            [x_state(:,10*k+2),P_cov(:,:,10*k+2),K_EKF_gain(:,10*k+2)]=KF_form(x_vec_all(100,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,3),x_state(:,10*k+1),P_cov(:,:,10*k+1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+2)=x_state(:,10*k+1);
            P_cov(:,:,10*k+2)=P_cov(:,:,10*k+1);
            K_EKF_gain(:,10*k+2)=K_EKF_gain(:,10*k+1);       
...
if (k>200)
            [x_state(:,10*k+3),P_cov(:,:,10*k+3),K_EKF_gain(:,10*k+3)]=KF_form(x_vec_all(200,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,4),x_state(:,10*k+2),P_cov(:,:,10*k+2),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+3)=x_state(:,10*k+2);
            P_cov(:,:,10*k+3)=P_cov(:,:,10*k+2);
            K_EKF_gain(:,10*k+3)=K_EKF_gain(:,10*k+2);   
...
if (k>300)
            [x_state(:,10*k+4),P_cov(:,:,10*k+4),K_EKF_gain(:,10*k+4)]=KF_form(x_vec_all(300,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,5),x_state(:,10*k+3),P_cov(:,:,10*k+3),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+4)=x_state(:,10*k+3);
            P_cov(:,:,10*k+4)=P_cov(:,:,10*k+3);
            K_EKF_gain(:,10*k+4)=K_EKF_gain(:,10*k+3);   
...
if (k>400)
            [x_state(:,10*k+5),P_cov(:,:,10*k+5),K_EKF_gain(:,10*k+5)]=KF_form(x_vec_all(400,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,6),x_state(:,10*k+4),P_cov(:,:,10*k+4),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+5)=x_state(:,10*k+4);
            P_cov(:,:,10*k+5)=P_cov(:,:,10*k+4);
            K_EKF_gain(:,10*k+5)=K_EKF_gain(:,10*k+4);
...
if (k>500)
            [x_state(:,10*k+6),P_cov(:,:,10*k+6),K_EKF_gain(:,10*k+6)]=KF_form(x_vec_all(500,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,7),x_state(:,10*k+5),P_cov(:,:,10*k+5),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+6)=x_state(:,10*k+5);
            P_cov(:,:,10*k+6)=P_cov(:,:,10*k+5);
            K_EKF_gain(:,10*k+6)=K_EKF_gain(:,10*k+5);
...
if (k>600)
            [x_state(:,10*k+7),P_cov(:,:,10*k+7),K_EKF_gain(:,10*k+7)]=KF_form(x_vec_all(600,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,8),x_state(:,10*k+6),P_cov(:,:,10*k+6),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+7)=x_state(:,10*k+6);
            P_cov(:,:,10*k+7)=P_cov(:,:,10*k+6);
            K_EKF_gain(:,10*k+7)=K_EKF_gain(:,10*k+6);
...
if (k>700)
            [x_state(:,10*k+8),P_cov(:,:,10*k+8),K_EKF_gain(:,10*k+8)]=KF_form(x_vec_all(700,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,9),x_state(:,10*k+7),P_cov(:,:,10*k+7),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+8)=x_state(:,10*k+7);
            P_cov(:,:,10*k+8)=P_cov(:,:,10*k+7);
            K_EKF_gain(:,10*k+8)=K_EKF_gain(:,10*k+7);
...
if (k>800)
            [x_state(:,10*k+9),P_cov(:,:,10*k+9),K_EKF_gain(:,10*k+9)]=KF_form(x_vec_all(800,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,10),x_state(:,10*k+8),P_cov(:,:,10*k+8),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state(:,10*k+9)=x_state(:,10*k+8);
            P_cov(:,:,10*k+9)=P_cov(:,:,10*k+8);
            K_EKF_gain(:,10*k+9)=K_EKF_gain(:,10*k+8);
...
%     x_vec_all_plot(10*k-9,:)=x_vec_all(k,:);
%     x_vec_all_plot(10*k-8,:)=x_vec_all(k,:);
% 	x_vec_all_plot(10*k-7,:)=x_vec_all(k,:);
% 	x_vec_all_plot(10*k-6,:)=x_vec_all(k,:);
%     x_vec_all_plot(10*k-5,:)=x_vec_all(k,:);
%     x_vec_all_plot(10*k-4,:)=x_vec_all(k,:);
% 	x_vec_all_plot(10*k-3,:)=x_vec_all(k,:);
%     x_vec_all_plot(10*k-2,:)=x_vec_all(k,:);
%     x_vec_all_plot(10*k-1,:)=x_vec_all(k,:);
% 	x_vec_all_plot(10*k,:)=x_vec_all(k,:);
%     
%     plot_animation_search(N_plots,10*k-9,x_t_vec,x_vec_all_plot(1:(10*k-9),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k)),k_obs,N_loops_fb,P_cov(:,:,(10*k)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-8,x_t_vec,x_vec_all_plot(1:(10*k-8),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+1)),k_obs,N_loops_fb,P_cov(:,:,(10*k+1)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-7,x_t_vec,x_vec_all_plot(1:(10*k-7),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+2)),k_obs,N_loops_fb,P_cov(:,:,(10*k+2)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-6,x_t_vec,x_vec_all_plot(1:(10*k-6),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+3)),k_obs,N_loops_fb,P_cov(:,:,(10*k+3)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-5,x_t_vec,x_vec_all_plot(1:(10*k-5),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+4)),k_obs,N_loops_fb,P_cov(:,:,(10*k+4)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-4,x_t_vec,x_vec_all_plot(1:(10*k-4),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+5)),k_obs,N_loops_fb,P_cov(:,:,(10*k+5)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-3,x_t_vec,x_vec_all_plot(1:(10*k-3),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+6)),k_obs,N_loops_fb,P_cov(:,:,(10*k+6)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-2,x_t_vec,x_vec_all_plot(1:(10*k-2),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+7)),k_obs,N_loops_fb,P_cov(:,:,(10*k+7)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k-1,x_t_vec,x_vec_all_plot(1:(10*k-1),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+8)),k_obs,N_loops_fb,P_cov(:,:,(10*k+8)),p_e,0,psi_jammer);
%     plot_animation_search(N_plots,10*k,x_t_vec,x_vec_all_plot(1:(10*k),:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:(10*k+9)),k_obs,N_loops_fb,P_cov(:,:,(10*k+9)),p_e,0,psi_jammer);
...
P_r_filt_ratio=[P_r_filt_ratio ; zeros(N_loops_vf-N_loops_fb,7)];           %   Alpha: Power ratio between initial and current : see 'alpha' in report
...
distance_trav(1)=norm(x_vec_all(1,:));

for k=2:N_loops_vf
if(k<ind) % The error is only computed when the KF is used
error_RMS_for_CR_extended(k)=0;
distance_trav(k)=distance_trav(k-1)+norm(x_vec_all(k,:)-x_vec_all(k-1,:));
else
    
error_RMS_for_CR_EKF_power_ratio(10*k-9)=sqrt( (x_state(1,10*k)-x_t_vec(1))^2+(x_state(2,10*k)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-8)=sqrt( (x_state(1,10*k+1)-x_t_vec(1))^2+(x_state(2,10*k+1)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-7)=sqrt( (x_state(1,10*k+2)-x_t_vec(1))^2+(x_state(2,10*k+2)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-6)=sqrt( (x_state(1,10*k+3)-x_t_vec(1))^2+(x_state(2,10*k+3)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-5)=sqrt( (x_state(1,10*k+4)-x_t_vec(1))^2+(x_state(2,10*k+4)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-4)=sqrt( (x_state(1,10*k+5)-x_t_vec(1))^2+(x_state(2,10*k+5)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-3)=sqrt( (x_state(1,10*k+6)-x_t_vec(1))^2+(x_state(2,10*k+6)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-2)=sqrt( (x_state(1,10*k+7)-x_t_vec(1))^2+(x_state(2,10*k+7)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k-1)=sqrt( (x_state(1,10*k+8)-x_t_vec(1))^2+(x_state(2,10*k+8)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio(10*k)=sqrt( (x_state(1,10*k+9)-x_t_vec(1))^2+(x_state(2,10*k+9)-x_t_vec(2))^2 );
...
% for k=2:10:10*N_loops_vf+9
% if(k<ind)
% cramer(k)=sqrt(abs(trace(P_cov_ini)));
% else
% cramer(k)=sqrt(abs(trace(P_cov(:,:,10*k))));
% end
% end

% %Plotting the RMSE as a function of the time step
time_step=10*ind:1:10*N_loops_vf;
% figure
% plot(time_step,error_RMS_for_CR_extended(ind:N_loops_vf),'r',time_step,cramer(ind:N_loops_vf),'g')
...
% 
% %Plotting the RMSE as a function of the distance travelled by the UAV
% figure
% plot(distance_trav(ind:N_loops_vf)/1000,error_RMS_for_CR_extended(ind:N_loops_vf),'r',distance_trav(ind:N_loops_vf)/1000,cramer(ind:N_loops_vf),'g')
% title('Root Mean Squared Error on the GPS Jammer position - Extended Kalman Filter')
% xlabel('Distance travelled by the UAV (km)')
% ylabel('Root Mean Squared Error (m)')

% figure
% plot(time_step,error_RMS_for_CR_EKF_power_ratio(1,10*ind:1:10*N_loops_vf),'r')
% title('Root Mean Squared Error on the GPS Jammer position - Extended Kalman Filter')
% xlabel('Time step')
% ylabel('Root Mean Squared Error (m)')
