...
%% New part to generate the particles
tic;
% initialisation
...
V_y=P_cov_ini(2,2);
    ...
    %Generate particles - with a variance, the initial guess for the
    %variance

for i=1:N
    x_P(i,1) = x + sqrt(V_x) * randn;
    y_P(i,1) = y + sqrt(V_y) * randn;
for k=1:50
x_P(i,k) =x_P(i,1);
y_P(i,k) = y_P(i,1);
end
end
time_generate_particle=toc ;  
%%
...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    %   Kalman filtering: EKF (UKF)
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))              	%   If intersections have begun & first time EKF (UKF) is run
            ...
            %Particle Filter
            tic;
            [x_P(:,k), y_P(:,k), x_est(k), y_est(k),x_P_update(:,k), y_P_update(:,k)] =particle_filter(x_P(:,k-1), y_P(:,k-1),N,x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,Q_KF,R_KF);
            
            x_state(:,k)=[x_est(k) ;
                          y_est(k)];
            time_comput_particle(k)=toc ;  
        ...
        elseif ((obs_check==1)&&(k>k_obs))                                  %   If intersections have begun & EKF (UKF) has alreay started
            ...
            % Particle Filter
            tic;
            [x_P(:,k), y_P(:,k), x_est(k), y_est(k),x_P_update(:,k), y_P_update(:,k)] =particle_filter(x_P(:,k-1), y_P(:,k-1),N,x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,k-1),P_cov_ini,Q_KF,R_KF);
...
%     plot_animation_search_particle_filter(N_plots,k,x_t_vec,x_vec_all(1:k,:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:k),k_obs,N_loops_fb,P_cov(:,:,k),p_e,0,psi_jammer,x_P(:,k),y_P(:,k));
end
...
%             [x_state(:,k), P_cov(:,:,k), K_UKF_gain(:,k)] = ukfmine(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,Q_KF,R_KF);
...
%             x_state(:,k) = x_t_vec;
    ...
    %   Animation: plot new UAV, Jammer and UAV trace at each iteration.
    %   See corresponding function for detail
%     plot_animation_search_particle_filter(N_plots,k,x_t_vec,x_vec_all(1:k,:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:k),k_obs,N_loops_fb,P_cov(:,:,k),p_e,r_d,psi_jammer,x_P(:,k),y_P(:,k));
...
error_RMS_for_CR_particle(k)=0;
distance_trav(k)=distance_trav(k-1)+norm(x_vec_all(k,:)-x_vec_all(k-1,:));
else
error_RMS_for_CR_particle(k)=sqrt( (x_state(1,k)-x_t_vec(1))^2+(x_state(2,k)-x_t_vec(2))^2 );
distance_trav(k)=distance_trav(k-1)+norm(x_vec_all(k,:)-x_vec_all(k-1,:));
end
end

% We don't have the Cramer-Rao Lower Bound as we don't calculate the
% covariance matrix.
...
% else
% cramer(k)=sqrt(abs(trace(P_cov(:,:,k))));
% end
% end

% %Plotting the RMSE as a function of the time step
time_step=1:1:N_loops_vf;
% figure
