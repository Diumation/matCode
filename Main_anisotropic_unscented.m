...
%             %---------- < Q_KF must be set up appropriately > ------------%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             P_cov_ini=diag([800^(2) 800^(2)]);                            %   Initial state covariance guess - Change if needed
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%             
...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    %   Filtering
        ...
        elseif ((obs_check==1)&&(k>k_obs))                                  %   If intersections have begun & filter has alreay started
            ...
            % Unscented Kalman Filter
            tic;
            [x_state(:,k), P_cov(:,:,k), K_UKF_gain(:,k)] = ukfmine(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,k-1),P_cov(:,:,k-1),Q_KF,R_KF);
            time_comput_UKF(k)=toc;
...
%%%% Students must analyse the performance of their own filters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Root Mean Square Error (RMSE) as a function of the time step

for k=1:N_loops_vf
if(k<ind) % The error is only computed when the KF is used
error_RMS_for_CR_unscented(k)=0;
else
error_RMS_for_CR_unscented(k)=sqrt( (x_state(1,k)-x_t_vec(1))^2+(x_state(2,k)-x_t_vec(2))^2 );
end
end
...
% figure
% plot(time_step,error_RMS_for_CR_unscented(ind:N_loops_vf),'r',time_step,cramer(ind:N_loops_vf),'g')
...
% figure
% plot(distance_trav(ind:N_loops_vf)/1000,error_RMS_for_CR_unscented(ind:N_loops_vf),'r',distance_trav(ind:N_loops_vf)/1000,cramer(ind:N_loops_vf),'g')
% title('Root Mean Squared Error on the GPS Jammer position - Unscented Kalman Filter')
