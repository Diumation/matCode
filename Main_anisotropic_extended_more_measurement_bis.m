...
if ((obs_check==1)&&(k==k_obs)||(re_run_bool==1)) %   If intersections have begun & first time filter is run  
            [x_state_1(:,k),P_cov_1(:,:,k),K_EKF_gain_1(:,k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else
            x_state_1(:,k)=x_state_ini;
            P_cov_1(:,:,k)=P_cov_ini;
            K_EKF_gain_1(:,k)=zeros(2,1);  
end
if ((obs_check==1)&&(k==50)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_50(:,k),P_cov_50(:,:,k),K_EKF_gain_50(:,k)]=KF_form(x_vec_all(50,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,2),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_50(:,k)=x_state_ini;
            P_cov_50(:,:,k)=P_cov_ini;
            K_EKF_gain_50(:,k)=zeros(2,1);          
end
if ((obs_check==1)&&(k==100)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_100(:,k),P_cov_100(:,:,k),K_EKF_gain_100(:,k)]=KF_form(x_vec_all(100,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,3),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_100(:,k)=x_state_ini;
            P_cov_100(:,:,k)=P_cov_ini;
            K_EKF_gain_100(:,k)=zeros(2,1);        
end
if ((obs_check==1)&&(k==200)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_200(:,k),P_cov_200(:,:,k),K_EKF_gain_200(:,k)]=KF_form(x_vec_all(200,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,4),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_200(:,k)=x_state_ini;
            P_cov_200(:,:,k)=P_cov_ini;
            K_EKF_gain_200(:,k)=zeros(2,1);      
end 
if ((obs_check==1)&&(k==300)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_300(:,k),P_cov_300(:,:,k),K_EKF_gain_300(:,k)]=KF_form(x_vec_all(300,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,5),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_300(:,k)=x_state_ini;
            P_cov_300(:,:,k)=P_cov_ini;
            K_EKF_gain_300(:,k)=zeros(2,1);      
end
if ((obs_check==1)&&(k==400)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_400(:,k),P_cov_400(:,:,k),K_EKF_gain_400(:,k)]=KF_form(x_vec_all(400,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,6),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_400(:,k)=x_state_ini;
            P_cov_400(:,:,k)=P_cov_ini;
            K_EKF_gain_400(:,k)=zeros(2,1);      
end
if ((obs_check==1)&&(k==500)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_500(:,k),P_cov_500(:,:,k),K_EKF_gain_500(:,k)]=KF_form(x_vec_all(500,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,7),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_500(:,k)=x_state_ini;
            P_cov_500(:,:,k)=P_cov_ini;
            K_EKF_gain_500(:,k)=zeros(2,1);      
end
if ((obs_check==1)&&(k==600)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_600(:,k),P_cov_600(:,:,k),K_EKF_gain_600(:,k)]=KF_form(x_vec_all(600,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,8),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_600(:,k)=x_state_ini;
            P_cov_600(:,:,k)=P_cov_ini;
            K_EKF_gain_600(:,k)=zeros(2,1);      
end
if ((obs_check==1)&&(k==700)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_700(:,k),P_cov_700(:,:,k),K_EKF_gain_700(:,k)]=KF_form(x_vec_all(700,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,9),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_700(:,k)=x_state_ini;
            P_cov_700(:,:,k)=P_cov_ini;
            K_EKF_gain_700(:,k)=zeros(2,1);      
end
if ((obs_check==1)&&(k==800)||(re_run_bool==1)) %   If intersections have begun & first time filter is run
            [x_state_800(:,k),P_cov_800(:,:,k),K_EKF_gain_800(:,k)]=KF_form(x_vec_all(800,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,10),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_800(:,k)=x_state_ini;
            P_cov_800(:,:,k)=P_cov_ini;
            K_EKF_gain_800(:,k)=zeros(2,1);      
end
            time_comput_EKFPR_bis(k)=toc ;
        ...
        if ((obs_check==1)&&(k>k_obs))                                  %   If intersections have begun & filter has alreay started
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %-------- < check the performance of the EKF (UKF) > ---------%
            %------------- < Design a new filer algorithm > --------------%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,k-1),P_cov(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
            [x_state_1(:,k),P_cov_1(:,:,k),K_EKF_gain_1(:,k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_1(:,k-1),P_cov_1(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
if (k>50)
            [x_state_50(:,k),P_cov_50(:,:,k),K_EKF_gain_50(:,k)]=KF_form(x_vec_all(50,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,2),x_state_50(:,k-1),P_cov_50(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_50(:,k)=x_state_ini;
            P_cov_50(:,:,k)=P_cov_ini;
            K_EKF_gain_50(:,k)=zeros(2,1);          
...
if (k>100)
            [x_state_100(:,k),P_cov_100(:,:,k),K_EKF_gain_100(:,k)]=KF_form(x_vec_all(100,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,3),x_state_100(:,k-1),P_cov_100(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_100(:,k)=x_state_ini;
            P_cov_100(:,:,k)=P_cov_ini;
            K_EKF_gain_100(:,k)=zeros(2,1);        
...
if (k>200)
            [x_state_200(:,k),P_cov_200(:,:,k),K_EKF_gain_200(:,k)]=KF_form(x_vec_all(200,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,4),x_state_200(:,k-1),P_cov_200(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_200(:,k)=x_state_ini;
            P_cov_200(:,:,k)=P_cov_ini;
            K_EKF_gain_200(:,k)=zeros(2,1);      
...
if (k>300)
            [x_state_300(:,k),P_cov_300(:,:,k),K_EKF_gain_300(:,k)]=KF_form(x_vec_all(300,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,5),x_state_300(:,k-1),P_cov_300(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_300(:,k)=x_state_ini;
            P_cov_300(:,:,k)=P_cov_ini;
            K_EKF_gain_300(:,k)=zeros(2,1);      
...
if (k>400)
            [x_state_400(:,k),P_cov_400(:,:,k),K_EKF_gain_400(:,k)]=KF_form(x_vec_all(400,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,6),x_state_400(:,k-1),P_cov_400(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_400(:,k)=x_state_ini;
            P_cov_400(:,:,k)=P_cov_ini;
            K_EKF_gain_400(:,k)=zeros(2,1);      
...
if (k>500)
            [x_state_500(:,k),P_cov_500(:,:,k),K_EKF_gain_500(:,k)]=KF_form(x_vec_all(500,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,7),x_state_500(:,k-1),P_cov_500(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_500(:,k)=x_state_ini;
            P_cov_500(:,:,k)=P_cov_ini;
            K_EKF_gain_500(:,k)=zeros(2,1);      
...
if (k>600)
            [x_state_600(:,k),P_cov_600(:,:,k),K_EKF_gain_600(:,k)]=KF_form(x_vec_all(600,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,8),x_state_600(:,k-1),P_cov_600(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_600(:,k)=x_state_ini;
            P_cov_600(:,:,k)=P_cov_ini;
            K_EKF_gain_600(:,k)=zeros(2,1);      
...
if (k>700)
            [x_state_700(:,k),P_cov_700(:,:,k),K_EKF_gain_700(:,k)]=KF_form(x_vec_all(700,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,9),x_state_700(:,k-1),P_cov_700(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_700(:,k)=x_state_ini;
            P_cov_700(:,:,k)=P_cov_ini;
            K_EKF_gain_700(:,k)=zeros(2,1);      
...
if (k>800)
            [x_state_800(:,k),P_cov_800(:,:,k),K_EKF_gain_800(:,k)]=KF_form(x_vec_all(800,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,10),x_state_800(:,k-1),P_cov_800(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
else 
            x_state_800(:,k)=x_state_ini;
            P_cov_800(:,:,k)=P_cov_ini;
            K_EKF_gain_800(:,k)=zeros(2,1);      
        ...
        %   Fusion of the data
    tic;
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))
if (k<50)
    x_state(:,k)=x_state_1(:,k);
    P_cov(:,:,k)=P_cov_1(:,:,k);
elseif (k<100)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;
elseif (k<200)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(1,1,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;
elseif (k<300)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;
elseif (k<400)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k),x_state_300(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k),P_cov_300(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k),P_cov_300(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;    
elseif (k<500)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k),x_state_300(:,k),x_state_400(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k),P_cov_300(1,1,k),P_cov_400(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k),P_cov_300(2,2,k),P_cov_400(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;      
elseif (k<600)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k),x_state_300(:,k),x_state_400(:,k),x_state_500(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k),P_cov_300(1,1,k),P_cov_400(1,1,k),P_cov_500(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k),P_cov_300(2,2,k),P_cov_400(2,2,k),P_cov_500(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;    
elseif (k<700)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k),x_state_300(:,k),x_state_400(:,k),x_state_500(:,k),x_state_600(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k),P_cov_300(1,1,k),P_cov_400(1,1,k),P_cov_500(1,1,k),P_cov_600(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k),P_cov_300(2,2,k),P_cov_400(2,2,k),P_cov_500(2,2,k),P_cov_600(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;    
elseif (k<800)
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k),x_state_300(:,k),x_state_400(:,k),x_state_500(:,k),x_state_600(:,k),x_state_700(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k),P_cov_300(1,1,k),P_cov_400(1,1,k),P_cov_500(1,1,k),P_cov_600(1,1,k),P_cov_700(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k),P_cov_300(2,2,k),P_cov_400(2,2,k),P_cov_500(2,2,k),P_cov_600(2,2,k),P_cov_700(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;    
else
    x_state(:,k)=mean([x_state_1(:,k),x_state_50(:,k),x_state_100(:,k),x_state_200(:,k),x_state_300(:,k),x_state_400(:,k),x_state_500(:,k),x_state_600(:,k),x_state_700(:,k),x_state_800(:,k)],2);
    P_cov(1,1,k)=mean([P_cov_1(1,1,k),P_cov_50(1,1,k),P_cov_100(1,1,k),P_cov_200(1,1,k),P_cov_300(1,1,k),P_cov_400(1,1,k),P_cov_500(1,1,k),P_cov_600(1,1,k),P_cov_700(1,1,k),P_cov_800(1,1,k)]);
    P_cov(2,2,k)=mean([P_cov_1(2,2,k),P_cov_50(2,2,k),P_cov_100(2,2,k),P_cov_200(2,2,k),P_cov_300(2,2,k),P_cov_400(2,2,k),P_cov_500(2,2,k),P_cov_600(2,2,k),P_cov_700(2,2,k),P_cov_800(2,2,k)]);
    P_cov(2,1,k)=0;P_cov(1,2,k)=0;   
end
        time_to_fuse(k)=toc ;
        end
...
error_RMS_for_CR_EKF_power_ratio_bis(N_loops_fb)=sqrt( (x_state(1,N_loops_fb)-x_t_vec(1))^2+(x_state(2,N_loops_fb)-x_t_vec(2))^2 );
error_RMS_for_CR_EKF_power_ratio_bis(N_loops_vf)=sqrt( (x_state(1,N_loops_vf)-x_t_vec(1))^2+(x_state(2,N_loops_vf)-x_t_vec(2))^2 );
