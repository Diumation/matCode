function plot_animation_search(N_plots,C_loop,x_t_vec,x_vec,psi,r_est_l,r_est_h,centre_geo_circle,radius_geo_circle,x_state,k_obs,N_loops_fb,P_cov,p_e,r_d,psi_jammer,x_P,y_P)
...
%      plot(x_P/plot_scaling, y_P/plot_scaling,'bo');
end     %   End function
...
if isempty(h_geo_est_plot)
    h_geo_est_plot=plot((centre_geo_circle(:,1)+radius_geo_circle(:,1).*cos(theta_data))/plot_scaling,(centre_geo_circle(:,2)+radius_geo_circle(:,1).*sin(theta_data))/plot_scaling,'-k','Linewidth',2,'EraseMode','normal');
    ...
    if (rem(C_loop,floor(N_loops_fb/0.01))==0)
        plot((centre_geo_circle(:,1)+radius_geo_circle(:,1).*cos(theta_data))/plot_scaling,(centre_geo_circle(:,2)+radius_geo_circle(:,1).*sin(theta_data))/plot_scaling,'-k','Linewidth',2);
    end
end
end             %   End function
